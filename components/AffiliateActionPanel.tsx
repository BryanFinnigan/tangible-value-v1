...
export default function AffiliateActionPanel({ links }: { links: AffiliateLinks }) {
  ...
  return (
    <section className="wikiPanel affiliatePanel">
      <p className="eyebrow">Marketplace</p>
      <h2>Next Steps</h2>
      <div className="affiliateActions">
        {activeActions.map((action) => (
          <a key={action.key} href={action.href} target="_blank" rel="noopener noreferrer sponsored" className="affiliateButton">
            {action.label}
          </a>
        ))}
      </div>
      <p className="finePrint">
        Some links may be affiliate links when active. Educational content remains separate from marketplace availability.
      </p>
    </section>
  );
}
